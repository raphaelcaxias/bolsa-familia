...
html, body, .stApp { color: #1A2B4C !important; font-family: 'Inter', sans-serif !important; }
#MainMenu, footer, header { visibility: hidden; }
.block-container { padding: 2rem 2.5rem !important; max-width: 1400px !important; }

/* Header */
.page-header { display: flex; align-items: flex-end; justify-content: space-between; border-bottom: 1px solid #E8ECF0; padding-bottom: 20px; margin-bottom: 28px; }
.page-header-left h1 { font-size: 28px !important; font-weight: 700 !important; color: #1A2B4C !important; margin: 0 0 4px 0 !important; }
.page-header-left p { color: #6B7A8F; font-size: 14px; }
.badge { background: #E8F4FD; color: #0A66C2; padding: 4px 12px; border-radius: 20px; font-size: 11px; font-weight: 600; }

/* Cards */
.card { background: white; border-radius: 16px; padding: 24px; border: 1px solid #E8ECF0; margin-bottom: 20px; }
.card-title { font-size: 16px; font-weight: 700; color: #1A2B4C; margin-bottom: 16px; }
.card-icon { font-size: 28px; margin-bottom: 12px; }

/* KPI Cards */
.kpi-grid { display: flex; gap: 20px; margin-bottom: 32px; flex-wrap: wrap; }
.kpi-card { flex: 1; min-width: 180px; background: white; border-radius: 16px; padding: 24px 20px; text-align: center; border: 1px solid #E8ECF0; box-shadow: 0 1px 2px rgba(0,0,0,0.03); }
.kpi-label { font-size: 12px; text-transform: uppercase; color: #6B7A8F; margin-bottom: 8px; font-weight: 600; letter-spacing: 0.5px; }
.kpi-value { font-size: 32px; font-weight: 700; color: #1A2B4C; margin: 8px 0; }
.kpi-sub { font-size: 11px; color: #6B7A8F; }

/* Upload Area */
.upload-area { background: white; border: 2px dashed #0A66C2; border-radius: 20px; padding: 50px; text-align: center; margin: 30px 0; }
.upload-icon { font-size: 56px; margin-bottom: 16px; }
.upload-title { font-size: 20px; font-weight: 700; color: #1A2B4C; margin-bottom: 8px; }
.upload-sub { font-size: 14px; color: #6B7A8F; margin-bottom: 20px; }

/* Info Cards */
.info-grid { display: flex; gap: 20px; margin: 20px 0; flex-wrap: wrap; }
.info-card { flex: 1; min-width: 200px; background: white; border-radius: 16px; padding: 20px; border: 1px solid #E8ECF0; }
.info-number { font-size: 28px; font-weight: 700; color: #0A66C2; }
.info-label { font-size: 13px; color: #6B7A8F; margin-top: 8px; }

/* Section */
.section-header { margin: 40px 0 20px 0; border-bottom: 1px solid #E8ECF0; padding-bottom: 10px; }
.section-header h2 { font-size: 20px; font-weight: 700; color: #1A2B4C; margin: 0; }
.section-tag { background: #E8F4FD; padding: 4px 12px; border-radius: 20px; font-size: 11px; color: #0A66C2; }

/* Tabs */
.stTabs [data-baseweb="tab-list"] { gap: 8px; }
.stTabs [data-baseweb="tab"] { background: white; border-radius: 12px; padding: 8px 24px; color: #6B7A8F; border: 1px solid #E8ECF0; }
.stTabs [aria-selected="true"] { background: #0A66C2; color: white; border: none; }

/* Botões */
.stDownloadButton button { background: #0A66C2 !important; color: white !important; border-radius: 30px !important; padding: 10px 24px !important; font-weight: 600 !important; border: none !important; width: 100% !important; }

/* Footer */
.footer { text-align: center; padding: 30px 0 20px; margin-top: 40px; border-top: 1px solid #E8ECF0; color: #6B7A8F; font-size: 12px; }
</style>
...
# ============================================================
# SIDEBAR (apenas informações, sem upload)
# ============================================================
with st.sidebar:
    st.markdown("""
    <div style="display:flex; align-items:center; gap:10px; margin-bottom:20px;">
        <span style="font-size:28px;">🔬</span>
        <span style="font-weight:800; font-size:20px; color:#0A66C2;">CNPq Analytics</span>
    </div>
    """, unsafe_allow_html=True)
...
<div class="page-header">
    ...
    <div class="page-header-right">
        <span class="badge">Dashboard Oficial</span>
    </div>
</div>
""", unsafe_allow_html=True)
...
# ÁREA DE UPLOAD (central, visível)
st.markdown("""
<div class="upload-area">
    <div class="upload-icon">📁</div>
    <div class="upload-title">Carregue o arquivo CSV</div>
    <div class="upload-sub">Arquivo de bolsas do CNPq (até 200MB)</div>
</div>
""", unsafe_allow_html=True)

# File uploader (escondemos o label padrão)
uploaded_file = st.file_uploader("Selecione o CSV", type=["csv"], label_visibility="collapsed")
            ...
            if 'grande_area' in df_filtrado.columns:
                ...
                with col1:
                    ...
                    fig = px.bar(area_data, x='Valor', y='Área', orientation='h', color='Valor', 
                                color_continuous_scale=['#6B7A8F', '#0A66C2'], text='Valor')
            ...
            if 'regiao_nome' in df_filtrado.columns:
                ...
                with col2:
                    ...
                    if 'Sudeste' in reg_data['Região'].values and 'Norte' in reg_data['Região'].values:
                        ...
                        st.info(f"⚖️ **Sudeste** recebe **{diferenca:.1f}x mais** investimento que a região **Norte**")
...
Bolsas: {fmt_num(n_bolsas)}"""
    ...
    # ============================================================
    # TELA INICIAL (sem gráficos falsos)
    # ============================================================
    
    st.markdown("""
    <div style="background: white; border-radius: 20px; padding: 30px; border: 1px solid #E8ECF0; margin-bottom: 30px;">
        <h3 style="color: #1A2B4C; margin-bottom: 16px;">📋 Sobre este Dashboard</h3>
        <p style="color: #6B7A8F;">Este dashboard analisa dados oficiais do CNPq sobre bolsas de pesquisa no Brasil, abrangendo <strong>213.735 registros</strong> e mais de <strong>R$ 1 bilhão</strong> em investimentos entre 2014 e 2027.</p>
        <p style="color: #6B7A8F; margin-top: 12px;"><strong>✅ Fonte oficial:</strong> Portal Brasileiro de Dados Abertos (CGU/MDS)</p>
    </div>
    """, unsafe_allow_html=True)
    
    # Cards de informações (sem dados falsos)
    st.markdown("""
    <div class="info-grid">
        <div class="info-card">
            <div class="info-number">213.735</div>
            <div class="info-label">Bolsas de pesquisa</div>
        </div>
        <div class="info-card">
            <div class="info-number">R$ 1,02B</div>
            <div class="info-label">Investimento total</div>
        </div>
        <div class="info-card">
            <div class="info-number">88.079</div>
            <div class="info-label">Pesquisadores</div>
        </div>
        <div class="info-card">
            <div class="info-number">4.281</div>
    ...
    # Como usar
    st.markdown("""
    <div style="background: #E8F4FD; border-radius: 16px; padding: 24px; margin-top: 20px;">
        <h4 style="color: #1A2B4C; margin-bottom: 12px;">🚀 Como usar</h4>
        <ol style="color: #6B7A8F; margin: 0; padding-left: 20px;">
            <li>Clique no botão acima para fazer upload do arquivo CSV</li>
            <li>Baixe o CSV original usando o link no menu lateral</li>
            <li>Após carregar, explore os filtros e gráficos interativos</li>
            <li>Exporte os dados filtrados em CSV ou gere relatório</li>
        </ol>
    </div>
    """, unsafe_allow_html=True)
