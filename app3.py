    ...
    if df is not None and len(df) > 0:
        ...
        with col1:
            ...
            <div class="metric-card">
                ...
                <div class="metric-value">{fmt_brl(df_filtrado['valor_pago'].sum())}</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with col2:
            if 'grande_area' in df_filtrado.columns:
                ...
                <div class="insight-card">
                    <h4>🧬 Área Líder</h4>
                    <p>A área <b>{top_area}</b> é a que mais recebe investimentos.</p>
                </div>
                """, unsafe_allow_html=True)
        ...
        with col2:
            try:
                ...
                st.download_button("📊 Excel", output.getvalue(), f"dados_{datetime.now().strftime('%Y%m%d')}.xlsx", 
                                 "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
            except:
                st.info("Excel disponível localmente")
        
        with col3:
            relatorio = f"Total Investido: {fmt_brl(df_filtrado['valor_pago'].sum())}\nPesquisadores: {fmt_num(n_pesq)}\nData: {datetime.now().strftime('%d/%m/%Y')}"
